...
fun! <SID>toggle_send_on_save()
    if exists("s:ssos") && s:ssos == 0
        let s:ssos = 1
        au BufWritePost *.py :Python run_this_file()
        echo "Autosend On"
    else
        let s:ssos = 0
        au! BufWritePost *.py
...
command! -nargs=* IPython :Python km_from_string("<args>")
command! -nargs=0 IPythonClipboard :Python km_from_string(vim.eval('@+'))
command! -nargs=0 IPythonXSelection :Python km_from_string(vim.eval('@*'))
command! -nargs=* IPythonNew :Python new_ipy("<args>")
command! -nargs=* IPythonInterrupt :Python interrupt_kernel_hack("<args>")
command! -nargs=0 IPythonTerminate :Python terminate_kernel_hack()
...
#completions = [s.encode(vim_encoding) for s in matches] # PY3
